...
def carregar_dados():
    ...
    palavras_chave_total = [normalizar_texto(d) for d in df_top['Doença'].dropna().unique()]
    palavras_chave_graves = [normalizar_texto(d) for d in df_graves['Doença'].dropna().unique()]

    return df, palavras_chave_total, palavras_chave_graves

def treinar_modelos(df, features, features_eutanasia):
    le_mob = LabelEncoder()
    le_app = LabelEncoder()
...
def prever_melhorado(anamnese, modelos, le_mob, le_app, palavras_chave_completo, palavras_chave_graves, features, features_eutanasia):
    ...
    if any(frase in texto_norm for frase in ["nenhum apetite", "sem apetite", "perda de apetite", "nao come"]):
    ...
    if any(frase in texto_norm for frase in ["sem andar", "nao anda", "nao consegue andar", "nao caminha"]):
    ...
    peso_extra_eutanasia = 0.6 if tem_sintomas_terminais else 0.4 if tem_doenca_letal else 0.0
    ...
    return {
        ...
        "Doenças Graves Detectadas": graves_detectadas if graves_detectadas else "Nenhuma grave"
    }
